...
class HeatingStageSetTemp(HeatingStageParentCommand):
    ...
    cls_description = "Set heating stage temperature. Execution completes when stabilized."
...
class HeatingStageSetSetPoint(HeatingStageParentCommand):
    # cls_name = "heating_stage_set_temp"
    cls_description = "Set heating stage setpoint temperature. Execution completes immediately after setting setpoint, even if temperature is not stabilized."
    ...
    def execute(self) -> None:
        self._was_successful, self._result_message = self._receiver.set_settemp(self._temperature)
